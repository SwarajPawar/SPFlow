...
for dataset in datasets:
	...
	for i, output in enumerate(aspmn.learn_aspmn(train, test, get_stats=True, evaluate_parallel=True)):
		...
		plt.plot([optimal_meu]*len(meus), linewidth=3, color ="lime", label="Optimal MEU")
		plt.plot([original_stats["meu"]]*len(meus), linestyle="dotted", color ="red", label="LearnSPMN")
		...
		plt.plot([original_stats["nodes"]]*len(nodes), linestyle="dotted", color ="red", label="LearnSPMN")
