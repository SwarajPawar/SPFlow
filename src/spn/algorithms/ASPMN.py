...
class Anytime_SPMN:
	...
	def __init__(self, train_data , partial_order , decision_nodes, utility_node, feature_names,
		           util_to_bin = False ):
	...
	def learn_spmn_structure(self, train_data, index, scope_index):
		...
		if self.params.partial_order[index][0] in  self.params.decision_nodes:
		    ...
		    split_cols = get_split_cols_distributed_RDC_py()
	...
	def learn_aspmn(self):
		...
		spmn = learn_spmn_structure(train_data, index, scope_index)
