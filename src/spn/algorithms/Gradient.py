...
def leaf_gradient_backward(node, parent_result, gradient_result=None, lls_per_node=None, data=None):
...
def sum_gradient_backward(node, parent_result, gradient_result=None, lls_per_node=None, data=None):
    parent_gradients = merge_gradients(parent_result)
...
def prod_gradient_backward(node, parent_result, gradient_result=None, lls_per_node=None, data=None):
    parent_gradients = merge_gradients(parent_result)
...
def max_gradient_backward(node, parent_result, gradient_result=None, lls_per_node=None, data=None):
    parent_gradients = merge_gradients(parent_result)
    ...
    for i, c in enumerate(node.children):
        children_gradient = gradients + output_ll - lls_per_node[:, c.id]
        children_gradient[np.isinf(children_gradient)] = np.finfo(gradient_result.dtype).min
        children_gradient[data[:, node.dec_idx]!=node.dec_values[i]] = np.finfo(gradient_result.dtype).min    #no gradient unless in chosen path
        messages_to_children[c] = children_gradient
