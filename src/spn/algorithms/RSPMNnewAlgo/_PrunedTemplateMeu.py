...
def eval_rspmn_bottom_up_for_meu(self, template, data):
    ...
    for time_step_num_in_reverse_order in range(total_num_of_time_steps,
                                                -1, -1):
        ...
        each_time_step_data_for_template = \
            self.get_each_time_step_data_for_meu_pruned_template(
                data,
                time_step_num_in_reverse_order-1,
...
def select_actions(self, template, data,
    ...
    unrolled_meu_01, unrolled_likelihood_01 = meu_of_state(self, template, data,
                 unrolled_network_meu_per_node,
                 unrolled_network_likelihood_per_node)
    # top down traversal
    for time_step_num in range(2):

        # if time step is 0, evaluate top network
        if time_step_num == 0:

            prev_lls_per_node = unrolled_likelihood_01[0]

            lls_per_node = unrolled_likelihood_01[1]
            meu_per_node = unrolled_meu_01[1]

            each_time_step_data_for_template = \
                self.get_each_time_step_data_for_meu_pruned_template(
                    data, time_step_num - 1,
                    total_num_of_time_steps,
                    prev_lls_per_node,
                    initial_num_latent_interface_nodes,
                    num_variables_each_time_step,
            ...
            prev_lls_per_node = unrolled_network_likelihood_per_node[-3]

            lls_per_node = unrolled_likelihood_01[0]
            meu_per_node = unrolled_meu_01[0]
        ...
        if time_step_num == 1:
            data[
                :,
                (0 * num_variables_each_time_step):
                (0 * num_variables_each_time_step) +
                num_variables_each_time_step
                ] = \
                    each_time_step_data_for_template[:,
...
def meu_of_state(self, template, data,
    ...
    unrolled_likelihood_01 = []
    unrolled_meu_01 = []
    # evaluate template bottom up at each time step
    for time_step_num_in_reverse_order in range(1,
                                                -1, -1):
        ...
        if time_step_num_in_reverse_order == 0:
            ...
            prev_likelihood_per_node = unrolled_likelihood_01[-1]
            logging.debug(
                f'prev_likelihood_per_node {prev_likelihood_per_node.shape}')
            # print(f'prev_likelihood_per_node {prev_likelihood_per_node}')

            prev_meu_per_node = unrolled_meu_01[-1]
            logging.debug(f'prev_meu_per_node {prev_meu_per_node.shape}')
            meu_per_node = np.zeros((data.shape[0], len(template_nodes)))
            meu_per_node.fill(np.nan)
            ...
            prev_likelihood_per_node = unrolled_network_likelihood_per_node[-3]
            logging.debug(
                f'prev_likelihood_per_node {prev_likelihood_per_node.shape}')
            # print(f'prev_likelihood_per_node {prev_likelihood_per_node}')

            prev_meu_per_node = unrolled_network_meu_per_node[-3]
            logging.debug(f'prev_meu_per_node {prev_meu_per_node.shape}')
            meu_per_node = np.zeros((data.shape[0], len(template_nodes)))
            meu_per_node.fill(np.nan)
            likelihood_per_node = np.zeros((data.shape[0], len(template_nodes)))
        ...
        unrolled_likelihood_01.append(likelihood_per_node)
        unrolled_meu_01.append(meu_per_node)

    # print(unrolled_network_meu_per_node[-1][:, 0])

    return unrolled_meu_01, unrolled_likelihood_01