...
def rmeu(rspmn, input_data, depth, debug=False, gamma=1):
    ...
    for s2 in branch_s2s:
        ...
        s2_value = rmeu(rspmn, next_data, depth-1, gamma=gamma)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mi_threshold", default=0.3, type=float)
    ...
    parser.add_argument("--num_vars", default=14, type=int)#total number of columns in dataset

    args = parser.parse_args()
    args.mi_threshold *= 0.01
    print(args)
    ...
    df = pd.read_csv(
        datapath,
        index_col=0, sep='\t', nrows = args.samples,
    ...
    if args.dataset == "crossing_traffic":
        ...
        decisions = np.concatenate((np.zeros((decisions.shape[0],decisions.shape[1],1)),decisions),axis=2)
        decisions = np.argmax(decisions,axis=2)
        person_waiting0 = data[:,:,5]
        person_waiting1 = data[:,:,6]
        person_waiting2 = data[:,:,7]
        person_in_elevator_down = data[:,:,4]
        person_in_elevator_up = data[:,:,8]
        reward = data[:,:,-1]
        data = np.concatenate(
                (
                    decisions.reshape(data.shape[0],-1,1),
                    person_in_elevator_down.reshape(data.shape[0],-1,1),
                    person_waiting0.reshape(data.shape[0],-1,1),
                    person_waiting1.reshape(data.shape[0],-1,1),
                    person_waiting2.reshape(data.shape[0],-1,1),
    ...
    import pickle
    pkle_path = f"data/{args.dataset}/{args.samples}x{args.problem_depth}"
    ...
    if not path.exists(pkle_path):
        try:
            ...
            file = open(f"data/{args.dataset}/rspmn_{args.mi_threshold*100}_{args.horizon}.pkle",'wb')
            pickle.dump(rspmn, file)
            file.close()
            data_SIDs = train_data[:,:,0].reshape(args.samples,args.problem_depth)
            np.savetxt(f"data/{args.dataset}/data_SIDs.tsv", data_SIDs, delimiter='\t')
    if path.exists(pkle_path):
    
        file = open(f"{pkle_path}/rspmn_{int(args.mi_threshold*100)}_{args.horizon}.pkle",'wb')
        pickle.dump(rspmn, file)
        file.close()
        data_SIDs = train_data[:,:,0].reshape(args.samples,args.problem_depth)
    ...
    for i in range(1,args.problem_depth+1):
        print(f"rmeu for depth {i}:\t"+str(rmeu(rspmn, input_data, depth=i, gamma=0.95)))
    print(f"rmeu for depth 50:\t"+str(rmeu(rspmn, input_data, depth=50, gamma=0.95)))
    print(f"rmeu for depth 100:\t"+str(rmeu(rspmn, input_data, depth=100, gamma=0.95)))
    print(f"rmeu for depth 150:\t"+str(rmeu(rspmn, input_data, depth=150, gamma=0.95)))
    print(f"rmeu for depth 200:\t"+str(rmeu(rspmn, input_data, depth=200, gamma=0.95)))

    print("\napplying EM\n")
    clear_caches(rspmn)
    hard_em(rspmn, train_data)
    ...
    if path.exists(pkle_path):
        file = open(f"{pkle_path}/rspmn_EM_{int(args.mi_threshold*100)}_{args.horizon}.pkle",'wb')
        pickle.dump(rspmn, file)
        file.close()
